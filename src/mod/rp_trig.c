...
static int rp_trig_probe(struct platform_device *pdev) {
...
static int rp_trig_remove(struct platform_device *pdev) {
...
static const struct of_device_id rp_trig_of_ids[] = {
    { .compatible = "xlnx,w7x-timing-1.0",},
	{}
};

static struct platform_driver rp_trig_driver = {
	.driver = {
        .name  = MODULE_NAME,
		.owner = THIS_MODULE,
        .of_match_table = rp_trig_of_ids,
	},
    .probe = rp_trig_probe,
    .remove = rp_trig_remove,
};

static int __init rp_trig_init(void)
{
    printk(KERN_INFO "inizializing AXI rp_trig module ...\n");
    return platform_driver_register(&rp_trig_driver);
}

static void __exit rp_trig_exit(void)
{
    printk(KERN_INFO "exiting AXI rp_trig module ...\n");
    platform_driver_unregister(&rp_trig_driver);
}

module_init(rp_trig_init);
module_exit(rp_trig_exit);
MODULE_LICENSE("GPL");
